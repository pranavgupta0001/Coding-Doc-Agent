...
class DriftMiner:
    ...
    def score(self, doc, sig, code, prior):
        prior_val = 0.10 if prior else 0.0

        # base pattern score
        if sig and not doc:
            base, signal = 0.90, "signature_changed_without_doc"
        elif code and not doc:
            base, signal = 0.70, "code_changed_without_doc"
        elif doc and not code and not sig:
            base, signal = 0.60, "doc_only_change_possible_repair"
        elif doc and code:
            base, signal = 0.35, "doc_and_code_changed_ambiguous"
        elif (not doc) and (not sig) and (not code):
            base, signal = 0.05, "no_changes"
        elif doc and sig and code:
            base, signal = 0.20, "all_changed_likely_coordinated"
        else:
            base, signal = 0.25, "other_mixed_pattern"

        return min(1.0, base + prior_val), signal
    ...
    def build_symbol_records(
        ...
        for sym in sorted(all_symbols):
            ...
            drift_fix_commit = keyword_matched

            score, signal = self.score(doc_changed, signature_changed, code_changed, drift_fix_commit)
            ...
            record = {
                ...
                "labels": {
                    ...
                    "drift_fix_commit": drift_fix_commit,
                    "drift_score": score,
                    "description": signal,
                }
            }
    ...
    def mine_repository(
        ...
        require_changed_overlap: bool = True,
        include_non_keyword_commits: bool = False
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for commit in commits:
                ...
                keyword_matched = self.is_drift_fixing_commit(msg)

                # Old behavior: only mine keyword commits
                if (not include_non_keyword_commits) and (not keyword_matched):
                    continue
...
def main():
    ...
    parser.add_argument(
        '--include-non-keyword-commits',
        action='store_true',
        help='Also mine commits that do NOT match drift keywords (makes drift_fix_commit vary)'
    )
    ...
    for repo in args.repos:
        recs = miner.mine_repository(
            ...
            require_changed_overlap=require_changed_overlap,
            include_non_keyword_commits=args.include_non_keyword_commits
        )
